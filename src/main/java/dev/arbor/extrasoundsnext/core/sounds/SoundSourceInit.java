...
import dev.arbor.extrasoundsnext.core.annotation.CategoryLoader;

import java.lang.annotation.AnnotationFormatError;
import java.lang.reflect.Field;
import java.util.*;

public final class SoundSourceInit {
    /**
    ...
    public static Pair<CategoryLoader, List<Field>> getCategories() {
        return Pair.of(dev.arbor.extrasoundsnext.core.sounds.Mixers.class, getRegistrations());
    }

    public static List<Field> getRegistrations() {
        return Arrays.stream(dev.arbor.extrasoundsnext.core.sounds.Mixers.class.getDeclaredFields()).filter(it ->
                it.isAnnotationPresent(dev.arbor.extrasoundsnext.core.annotation.CategoryLoader.Register.class)).toList();
    }
    ...
    public static void initCategoryLoader() {
        ...
        //required so that new categories are actually created, not used
        SoundSource.MASTER.getClass().getClassLoader();

        try {
            final Pair<dev.arbor.extrasoundsnext.core.annotation.CategoryLoader, List<Field>> allAnnotations = getCategories();

            // First fetch for MASTER categories.
            for (Field field : allAnnotations.getSecond()) {
                final dev.arbor.extrasoundsnext.core.annotation.CategoryLoader categoryLoader = allAnnotations.getFirst();
                final dev.arbor.extrasoundsnext.core.annotation.CategoryLoader.Register annotation = field.getAnnotation(dev.arbor.extrasoundsnext.core.annotation.CategoryLoader.Register.class);
