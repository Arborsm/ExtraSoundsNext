...
public final class SoundSouceInit {
    ...
    public static Pair<CategoryLoader, List<Field>> getCategories() {
        return Pair.of(ExtraSounds.MIXERS, getRegistrations());
    }
    ...
    public static void initCategoryLoader() {
        ...
        try {
            ...
            for (Field field : allAnnotations.getSecond()) {
                ...
                if (!annotation.master()) {
                    if (MASTERS.containsKey(className)) {
                        ...
                        if (!SUPPRESSED_NAMES.contains(className)) {
                            ExtraSounds.LOGGER.warn("[{}] Missing annotation value 'master' in class '{}'. This is deprecated.",
                                    ExtraSounds.class.getSimpleName(), className);
                            ExtraSounds.LOGGER.warn("[{}] To avoid this message, please specify \"master = true\" in one of the @Register annotation in your class.",
            ...
            ExtraSounds.LOGGER.error("[%s] Unexpected error has caught".formatted(ExtraSounds.class.getSimpleName()), ex);
        }
